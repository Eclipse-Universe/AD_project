"""GMM + KMeans-Mahal + LOF 3모델 앙상블.

Exp24(GMM+Mahal α=0.5, F1=0.9620) 기반으로 LOF를 추가해 blind spot 보완 시도.
LOF는 Exp15(F1=0.9237)와 동일 설정: standard scaler, n_neighbors=20, 52 피처.

가중치 탐색 (β_gmm, β_mahal, β_lof):
  GMM:Mahal 비율을 0.5:0.5로 고정하고 LOF 비중을 0→30% 구간에서 탐색.
  각 점수는 Z-score 정규화 후 합산, 상위 32%를 이상 판정.

실행: cd src && python run_ensemble3.py
"""
...
def predict(ens_test, run_index, test_run_ids, test_X_index):
...
def compare(pred_rows, ref_files):
    ...
    for name, path in ref_files.items():
        ...
        parts.append(f"{name}:{agree}/740(내+{me_only}/{name}+{ref_only})")
    return "  " + "  ".join(parts)
...
def main():
    ...
    # ── LOF (Exp15-A와 동일: standard, n_neighbors=20, 52 피처)
    print("[LOF 학습 중]")
    ...
    best_sep, best_w = -np.inf, None
    run_index = test_vecs.index

    for w_gmm, w_mahal, w_lof in WEIGHT_GRID:
        ens_tr = w_gmm*gmm_tr_n + w_mahal*mahal_tr_n + w_lof*lof_tr_n
        ens_te = w_gmm*gmm_te_n + w_mahal*mahal_te_n + w_lof*lof_te_n

        sep = (ens_te.mean() - ens_tr.mean()) / ens_tr.std()
        pred = predict(ens_te, run_index, test_run_ids, test_X.index)

        tag = f"g{w_gmm}_m{w_mahal}_l{w_lof}"
        save_submission(pred, test_X.index, OUTPUT_DIR / f"output_ens3_{tag}.csv")

        print(f"\n[GMM={w_gmm} Mahal={w_mahal} LOF={w_lof}]  sep={sep:.2f}  pos={pred.mean():.4f}")
        print(compare(pred, ref_files))

        if sep > best_sep:
            best_sep = sep
            best_w   = (w_gmm, w_mahal, w_lof)

    print(f"\n→ sep 기준 최적 = GMM:{best_w[0]} Mahal:{best_w[1]} LOF:{best_w[2]}")
